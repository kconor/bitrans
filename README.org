...
#+end_src
  - Grab a transaction
    ...
    Note that if your bitcoind hasn't finished reindex, the current rpcserver will just return NoneType Error.
    Just grab a different txid or wait until reindex finishes.
#+begin_src py
>>> import transaction
