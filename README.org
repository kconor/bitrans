...
* Getting it running
  1. Install [[https://en.bitcoin.it/wiki/Bitcoind][bitcoind]]
  2. Enable the bitcoin rpc server in ~/bitcoin.conf by
...
python
#+end_src
  6. Establish a connection to the bitcoind daemon
#+begin_src py
>>> import rpc
>>> s = rpc.jsonrpc("yourUser","yourPass")
#+end_src
  7. Grab a transaction
#+begin_src py
>>> import transaction
